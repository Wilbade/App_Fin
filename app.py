...
# --- ABA 2: LANÇAR ---
with aba2:
    tipo = st.radio("Método:", ["✍️ Manual", "📸 Recibo Único", "📄 Fatura Completa"], horizontal=True)
    if tipo == "✍️ Manual":
        with st.form("f_manual"):
            c1, c2 = st.columns(2)
            u = c1.radio("Quem?", ["wil", "ju"], horizontal=True)
            d = c1.date_input("Data:", value=datetime.now())
            # Busca categorias das metas para o dropdown
            ...
            if st.form_submit_button("Salvar"):
                p = {"user_id":u, "data":str(d), "categoria":cat, "subcategoria":sub, "valor_planeado":val, "meio_pagamento":meio}
                supabase.table("transacoes").insert(p).execute()
                st.rerun()
    # (Scanner e Fatura continuam iguais, apenas o design foi mantido)
        ...
        if 'res_ia' in st.session_state:
            ...
            if st.button("Confirmar e Salvar"):
                p = {"user_id":"wil", "data":r['data'], "categoria":r['categoria'], "subcategoria":r['local'], "valor_planeado":r['valor'], "meio_pagamento":r['meio']}
                supabase.table("transacoes").insert(p).execute()
...
with aba4:
    if not df.empty:
        ...
        if not df_c.empty:
            st.error(f"Total Fatura: R$ {df_c['valor_planeado'].sum():,.2f}")
...
# --- ABA 5: HISTÓRICO (O QUE FOI MELHORADO) ---
with aba5:
    st.subheader("📝 Detalhes e Edição Profissional")
    if not df.empty:
        # Filtramos os dados do mês/ano selecionados
            ...
            if st.button("💾 Salvar Alterações"):
                with st.spinner("Atualizando banco..."):
                    for _, row in editado.iterrows():
                        # O ID continua no dataframe mas não aparece pro usuário
                        p = {
                            "user_id": row['user_id'],
                            "categoria": row['categoria'].upper(), # Padroniza para maiúscula
